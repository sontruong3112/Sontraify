...
## Common Deploy Error: `Couldn't find a package.json file in /opt/render/project/src`

If you created a **manual Web Service** (not Blueprint), Render may run from repo root and ignore monorepo folder structure.

Use these settings in Render backend service:

- Root Directory: `backend`
- Build Command: `npm install`
- Start Command: `npm start`

If you continue seeing `yarn start` in logs, Render is still using default command from root. In that case:

1. Open service Settings -> Build & Deploy and update commands above.
2. Or delete and recreate service via `New +` -> `Blueprint` so `render.yaml` is applied automatically.
